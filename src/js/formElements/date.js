...
module.exports = BaseElement.extend({
    ...
    setValueFromInput: function(e) {
        ...
        try {
            ...
            return this.setValue(this.$input.val() === '' ? '' : formatDate(parseDate(this.$input.val(), 'DD.MM.YYYY'), 'YYYY-MM-DD'));

        } catch (error) {

            console.log('Date value "' + this.$input.val() + '" is not in correct format!'); // eslint-disable-line no-console
    ...
    render: function() {
        ...
        this.$input = $('<input type="text" />').val(this.getValue() ? formatDate(this.getValue(), 'DD.MM.YYYY') : '');
